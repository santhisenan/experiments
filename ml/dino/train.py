...
from data_augmentations import DataAugmentations
from ViT import Head, Loss, MultiCropWrapper, gradient_clip
...
def main():
    ...
    transform_aug = DataAugmentations(size=224, n_local_crops=args.n_crops - 2)
    ...
    student = MultiCropWrapper(
        student_vit, Head(dim, args.out_dim, norm_last_layer=args.norm_last_layer)
    )
        ...
        for i, (images, _) in tqdm.tqdm(
            ...
            if n_steps % args.logging_freq == 0:
                ...
                embs, imgs, labels_ = compute_embedding(
                    student.backbone, data_loader_val_plain_subset
                )
                ...
                current_acc = compute_knn(
                    student.backbone, data_loader_train_plain, data_loader_val_plain
                )
            ...
            optimizer.zero_grad()
            loss.backward()
            gradient_clip(student, args.clip_grad)
            optimizer.step()
