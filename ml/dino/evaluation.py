...
def compute_knn(backbone, data_loader_train, data_loader_val):
    """Get CLS embeddings and perform KNN classification.

    Load all the embeddings in memory and use sklearn.

    Args:
        backbone (_type_): _description_
        data_loader_train (_type_): _description_
        data_loader_val (_type_): _description_
    """
    device = next(backbone.parameters()).device

    data_loaders = {"train": data_loader_train, "val": data_loader_val}

    lists = {"X_train": [], "y_train": [], "X_val": [], "y_val": []}
